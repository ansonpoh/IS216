function generate_board() {
    ...
    let hidden_table = "<table style='margin:auto;'>";
    for(i = 0; i < num_rows; i++) {
        hidden_table += "<tr>";
        for(j = 0; j < num_cols; j++) {
            hidden_table += "<td>";
            hidden_table += "<img src=cards/hidden.png />"
            counter++;
            hidden_table += "</td>"
        }
        hidden_table += "</tr>";
    ...
    document.getElementById('game-board').innerHTML = hidden_table;

    var board = document.getElementById("game-board");

    var content = document.createElement("div");
    content.innerHTML = result_str;
    ...
    board.querySelectorAll("td").forEach((td, index) => {
        td.addEventListener("click", () => {

            if (lock || flipped.some(f => f.td === td)) {
                return;
            }

            td.innerHTML = "";
            var img = document.createElement("img");
            img.src = card_array[index];
            td.appendChild(img);
            flipped.push({td, value:card_array[index]})

            if(flipped.length === 2) {
                lock = true;
                if(flipped[0].value === flipped[1].value) {
                    score ++;
                    flipped[0].td.style.opacity = "0.5";
                    flipped[1].td.style.opacity = "0.5";
                    flipped = [];
                    if(score === totalCards/2) {
                        scoreDisplay.innerHTML = "All matched, Congratulations!";
                    } else {
                        scoreDisplay.innerHTML = "Total score: " + score;
                    }
                    lock = false;
